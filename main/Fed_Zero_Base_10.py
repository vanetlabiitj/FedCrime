...
def zinb_loss(zero_prob, nb_mean, nb_disp, targets):
...
def train(net, trainloader, learning_rate, epochs: int):
    ...
    for epoch in range(epochs):
        ...
        for batch in trainloader:
            ...
            loss = zinb_loss(zero_prob, mu, dispersion, labels)
            loss.backward()
            optimizer.step()
...
def val(net, testloader):
    ...
    with torch.no_grad():
        for batch in testloader:
            ...
            loss += zinb_loss(zero_prob, mu, dispersion, labels).item()
            total += labels.size(0)
            predicted_labels = (zero_prob > 0.5).float()
            all_predictions.extend(predicted_labels.tolist())
            all_labels.extend(labels.tolist())
...
def evaluate(
    ...
    file_path = "../store_results/CHI/omega/evaluate_server_CHI_fedzero_omega.txt"
    # Open the file for writing
    with open(file_path, "a") as file:
