...
def zip_loss(poisson_mean, zero_prob, labels):
    # Zero-inflation component
    zero_mask = (labels == 0).float()  # Mask where labels are zero
    non_zero_mask = 1 - zero_mask  # Mask where labels are non-zero

    # Negative log-likelihood for zero-inflation
    log_likelihood_zeros = zero_mask * torch.log(zero_prob + 1e-6)  # +1e-6 to avoid log(0)

    # Poisson component for non-zero labels
    poisson_nll = torch.distributions.Poisson(poisson_mean).log_prob(labels)

    # Combine the components (Poisson loss only applied to non-zero labels)
    log_likelihood_poisson = non_zero_mask * torch.log(1 - zero_prob + 1e-6) + poisson_nll

    # Final loss is negative of log-likelihood
    total_loss = -torch.sum(log_likelihood_zeros + log_likelihood_poisson)
    return total_loss / labels.numel()  # Normalized by number of elements
