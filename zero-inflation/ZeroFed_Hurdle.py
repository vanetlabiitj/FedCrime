...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        p = self.sigmoid(self.fc_nb_mean(out))  # NB mean (ensured positive)
        n = self.softplus(self.fc_nb_disp(out))  # NB dispersion (positive via softplus)

        return n, p


def hurdle_loss(poisson_mean, zero_prob, labels):
    """
    Computes Hurdle loss for zero-inflated count data.

    Args:
        poisson_mean (Tensor): Poisson mean (lambda) for nonzero counts.
        zero_prob (Tensor): Probability of zero-inflation (sigmoid output).
        labels (Tensor): Observed count labels.

    Returns:
        Tensor: Hurdle loss (averaged over batch).
    """
    eps = 1e-6  # To prevent log(0)

    # Mask for zero and non-zero labels
    zero_mask = (labels == 0).float()
    non_zero_mask = 1 - zero_mask  # 1 for non-zero labels

    # --- 1. Binary Classification for Zero-Inflation ---
    log_likelihood_zeros = zero_mask * torch.log(zero_prob + eps)  # Predicting zero

    # --- 2. Poisson Likelihood for Non-Zero Counts ---
    poisson_nll = torch.distributions.Poisson(poisson_mean).log_prob(labels)
    log_likelihood_poisson = non_zero_mask * (torch.log(1 - zero_prob + eps) + poisson_nll)

    # --- Final Hurdle Loss (Negative Log-Likelihood) ---
    total_loss = -torch.sum(log_likelihood_zeros + log_likelihood_poisson)

    return total_loss / labels.numel()  # Normalize by batch size
...
def val(net, testloader):
    ...
    with torch.no_grad():
        for batch in testloader:
            ...
            n, p = net(features)
            loss += hurdle_loss(n, p, labels).item()
            total += labels.size(0)
            predicted_labels = (p > 0.5).float()
            all_predictions.extend(predicted_labels.tolist())
            all_labels.extend(labels.tolist())
