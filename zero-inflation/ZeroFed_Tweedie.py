...
def load_datasets():
    ...
    for partition_id, partition_df in group_partition:
      if partition_id in [16, 11, 34, 30, 33, 36, 19, 13, 25, 2]:
...
class Net(nn.Module):
    def __init__(self, num_inputs=8, hidden_units=16,  num_layers=3, kernel_size=3):
        ...
        self.fc_nb_phi = nn.Linear(hidden_units, 8)
        self.fc_nb_rou = nn.Linear(hidden_units, 8)
    ...
    def forward(self, x):
        ...
        p = self.sigmoid(self.fc_nb_mean(out))
        n = self.softplus(self.fc_nb_disp(out))
        pi = self.sigmoid(self.fc_nb_phi(out))
        zi = self.sigmoid(self.fc_nb_rou(out))
        return pi, zi, n, p


def tweedie_loss(phi, rou, mu, zero_prob, labels):
    """
    Computes a multi-label Tweedie NLL loss with a zero-inflation component.

    Args:
        phi (Tensor): Dispersion parameter (must be positive).
        rou (Tensor): Tweedie power parameter (1 < rou < 2).
        mu (Tensor): Predicted mean values (must be positive).
        zero_prob (Tensor): Probability of excess zeros (sigmoid activated).
        labels (Tensor): Observed labels (multi-label, non-negative).

    Returns:
        Tensor: Mean Tweedie NLL loss.
    """
    eps = 1e-6  # Small value to prevent log(0) and numerical instability

    # Ensure valid parameter ranges
    rou = torch.clamp(rou, 1 + eps, 2 - eps)  # Tweedie power (1,2)
    phi = torch.clamp(phi, eps, 10)  # Dispersion (bounded for stability)
    mu = torch.clamp(mu, eps, None)  # Ensure positivity for mean prediction
    zero_prob = torch.sigmoid(zero_prob)  # Zero-inflation probability in (0,1)

    # Masks for zero and non-zero labels
    zero_mask = (labels == 0).float()
    non_zero_mask = 1 - zero_mask

    # --- Zero-Inflation Component ---
    log_likelihood_zeros = zero_mask * torch.log(zero_prob + eps)

    # --- Tweedie Negative Log-Likelihood for Non-Zero Values ---
    term1 = (labels ** (2 - rou)) / ((1 - rou) * (2 - rou) * phi)  # First term (includes phi)
    term2 = (labels * mu ** (1 - rou)) / ((1 - rou) * phi)  # Second term (includes phi)
    term3 = (mu ** (2 - rou)) / ((2 - rou) * phi)  # Third term (includes phi)

    log_likelihood_tweedie = term1 - term2 + term3

    # Weight Tweedie loss for non-zero labels
    log_likelihood_tweedie = non_zero_mask * (torch.log(1 - zero_prob + eps) - log_likelihood_tweedie)

    # --- Final loss: negative log-likelihood ---
    total_loss = -torch.sum(log_likelihood_zeros + log_likelihood_tweedie)
    return total_loss / labels.numel()  # Normalize over elements
...
def train(net, trainloader, learning_rate, epochs: int):
    ...
    for epoch in range(epochs):
        ...
        for batch in trainloader:
            ...
            optimizer.zero_grad()
            pi, zi, n, p = net(x)
            loss = tweedie_loss(pi, zi, n, p, labels)
            loss.backward()
            optimizer.step()
            epoch_loss_train += loss.item()
            total_train += labels.size(0)
            predicted_labels = (p > 0.5).float()
...
def val(net, testloader):
    ...
    with torch.no_grad():
        for batch in testloader:
            ...
            pi, zi, n, p = net(features)
            loss += tweedie_loss(pi, zi, n, p, labels).item()
            total += labels.size(0)
            predicted_labels = (p > 0.5).float()
            all_predictions.extend(predicted_labels.tolist())
            all_labels.extend(labels.tolist())
