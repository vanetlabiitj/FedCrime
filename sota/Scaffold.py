...
from flwr.server.strategy.aggregate import aggregate, aggregate_median, weighted_loss_avg, aggregate_krum, \
...
def load_datasets():
    ...
    for partition_id, partition_df in group_partition:
        if partition_id in [16, 11, 32, 18, 9, 74, 6, 47, 17, 28]:
...
def train(net, trainloader, learning_rate, global_c, local_c, epochs: int):
    ...
    for epoch in range(epochs):
        ...
        for batch in trainloader:
            ...
            loss.backward()
            for param, gc, lc in zip(net.parameters(), global_c, local_c):
                if param.grad is not None:
                    param.grad = param.grad - (lc + gc)
            optimizer.step()
...
class FlowerNumPyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        before_update_params = [param.clone().detach() for param in self.net.parameters()]

        file_path_control = os.path.join("../store_weights", f"server_control.pkl")
        file_path_local_control = os.path.join("../store_weights", f"cid-{self.cid}-local-c-x.pkl")

        if server_round == 1:
            self.global_c = [torch.zeros_like(param).to(DEVICE) for param in self.net.parameters()]
            self.local_c = [torch.zeros_like(param).to(DEVICE) for param in self.net.parameters()]

        else:
            with open(file_path_control, "rb") as f:
                self.global_c = pickle.load(f)
            with open(file_path_local_control, "rb") as f:
                self.local_c = pickle.load(f)


        train_loss, train_micro, train_macro = train(self.net, self.trainloader, learning_r, self.global_c, self.local_c, epochs=5)
        after_update_params = [param.clone().detach() for param in self.net.parameters()]
        delta_x = []
        for local_param, global_param in zip(after_update_params, before_update_params):
            delta_x.append((local_param - global_param).cpu().detach().numpy())

        local_parm_star = []
        for local_param, global_param, lc, gc in zip(after_update_params, before_update_params, self.local_c,
                                                     self.global_c):
            # lc -= global_param + (1 / (learning_r * len(self.trainloader.dataset))) * (global_param - local_param)
            lo = lc - global_param + (1 / (learning_r * len(self.trainloader.dataset))) * (global_param - local_param)
            local_parm_star.append(lo)

        delta_local_c = [lp - lc for lp, lc in zip(local_parm_star, self.local_c)]

        file_path = os.path.join("../store_weights", f"cid-{self.cid}-local-c-x.pkl")
        with open(file_path, "wb") as f:
            pickle.dump(local_parm_star, f)

        file_path = os.path.join("../store_weights", f"cid-{self.cid}-local-c.pkl")
        with open(file_path, "wb") as f:
            pickle.dump(delta_local_c, f)
        ...
        return delta_x, len(self.trainloader), {"loss": float(train_loss), "micro": float(train_micro),
...
def evaluate(
    ...
    print(
        f"Server-side evaluation: round {server_round} loss {loss}, accuracy {accuracy}, micro {micro}, macro {macro}")
    file_path = "../store_results/CHI/omega/evaluate_server_CHI_sfold_omega.txt"
    # Open the file for writing
    with open(file_path, "a") as file:
...
class FedCrime(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(
        ...
        control_locals = []
        for cid in range(10):
            file_path_local_control = os.path.join("../store_weights", f"cid-{cid}-local-c.pkl")
            with open(file_path_local_control, "rb") as f:
                locals = pickle.load(f)
                temp = []
                for layer in locals:
                    locals_numpy = layer.detach().numpy()
                    temp.append(locals_numpy)
                control_locals.append(temp)

        control_locals_transposed = list(zip(*control_locals))

        # Aggregate controls for each layer across all cids
        aggregated_control = []
        for layer_weights in control_locals_transposed:
            layer_mean = np.mean(np.array(layer_weights), axis=0)
            aggregated_control.append(layer_mean)

        # global control variate update
        file_path_read_control = os.path.join("../store_weights", f"server_control.pkl")
        if server_round == 1:
            self.global_control_plus = []
            for layer in locals:
                temp = torch.zeros_like(layer)
                self.global_control_plus.append(temp)
        else:
            with open(file_path_read_control, "rb") as f:
                self.global_control_plus = pickle.load(f)

        # update global control
        self.global_control_plus += aggregated_control

        # store server control
        file_path_store_control = os.path.join("../store_weights", f"server_control.pkl")
        with open(file_path_store_control, "wb") as f:
            pickle.dump(self.global_control_plus, f)

        # update server weights
        file_path = os.path.join("../store_weights", f"server_weights.pkl")

        if server_round == 1:
            global_weights = []
            for layer in locals:
                # Create a tensor with the same shape as the layer
                temp = torch.empty_like(layer)
                # Apply a weight initialization method (e.g., Xavier uniform)
                if temp.ndimension() >= 2:
                    # Apply Xavier uniform initialization for weight tensors
                    torch.nn.init.xavier_uniform_(temp)
                else:
                    # Use a simpler initialization for 1D tensors (e.g., biases)
                    torch.nn.init.uniform_(temp, a=-0.1, b=0.1)
                global_weights.append(temp)
        ...
        eta_aggregated_parameters = []
        aggregated_parameters = aggregate(deltas_x)
        for item in aggregated_parameters:
            temp = 0.1 * item
            eta_aggregated_parameters.append(temp)

        global_weights_updated = [a + b for a, b in zip(global_weights, eta_aggregated_parameters)]

        # store server weights
        with open(file_path, "wb") as f:
            pickle.dump(global_weights_updated, f)

        parameters_aggregated = ndarrays_to_parameters(global_weights_updated)
