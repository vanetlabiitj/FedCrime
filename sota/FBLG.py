...
def load_datasets():
    ...
    df.to_csv("../store_weights/output.csv", index=False)
...
class FlowerNumPyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        file_path = os.path.join("../store_weights", f"cid-{self.cid}-loss.pkl")
...
def evaluate(
    ...
    file_path = "../store_results/CHI/omega/evaluate_server_CHI_fblg_omega.txt"
    # Open the file for writing
    with open(file_path, "a") as file:
...
def sample(self, available_clients, num_clients, shortest_dist, C_ratio):
    ...
    for cid in self.available_clients:
        file_path = os.path.join("../store_weights", f"cid-{cid}-loss.pkl")
    ...
    df = pd.read_csv("../store_weights/output.csv")
