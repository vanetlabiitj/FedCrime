...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        proj = self.linear(tcn_out)
        final = self.sigmoid(proj)
        return tcn_out, proj, final
...
def train(net, global_net, previous_net, trainloader, learning_rate, mu, temp, epochs: int):
    ...
    for epoch in range(epochs):
        ...
        for batch in trainloader:
            ...
            # pro1 is the representation by the current model
            _, pro1, outputs = net(x)
            # pro2 is the representation by the global model
            _, pro2, _ = global_net(x)
            # pro 3 is the representation by the previous model
            _, pro3, _ = previous_net(x)

            # nega is the negative pair
            tensor1 = F.normalize(pro1, dim=1)
            tensor2 = F.normalize(pro2, dim=1)
            tensor3 = F.normalize(pro3, dim=1)
            posi = (tensor1 * tensor2) / temp
            nega = (tensor1 * tensor3) / temp
            logits = posi / (posi + nega)
            loss2 = mu * criterion2(logits, labels)
...
class FlowerNumPyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if not os.path.exists(os.path.join("../store_weights", f"{self.cid}", "prev_net.pt")):
            prev_net = copy.deepcopy(self.net)
        else:
            # load previous model from dir
            prev_net.load_state_dict(torch.load(os.path.join("../store_weights", f"{self.cid}", "prev_net.pt")))
        global_net = copy.deepcopy(self.net)
        global_net.load_state_dict(self.net.state_dict())

        train_loss, train_micro, train_macro = train(self.net, global_net, prev_net, self.trainloader, learning_r, self.mu, self.temp, epochs=5)
        ...
        if not os.path.exists(os.path.join("../store_weights", f"{self.cid}")):
            os.makedirs(os.path.join("../store_weights", f"{self.cid}"))
        torch.save(self.net.state_dict(), os.path.join("../store_weights", f"{self.cid}", "prev_net.pt"))
...
def evaluate(
    ...
    file_path = "../store_results/CHI/omega/evaluate_server_CHI_moon_omega.txt"
    # Open the file for writing
    with open(file_path, "a") as file:
